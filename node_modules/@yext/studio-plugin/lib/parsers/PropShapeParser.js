...
import { ParsedTypeKind, } from "./helpers/TypeNodeParsingHelper";
/**
...
export default class PropShapeParser {
     ...
     */
    parseShape(identifier, onProp) {
        const parsedType = this.studioSourceFileParser.parseTypeReference(identifier);
        if (!parsedType) {
            return {};
        }
    ...
    toPropShape(rawProps, identifier, onProp) {
        const propShape = {};
    ...
    toPropMetadata(rawProp, identifier, onProp) {
    ...
    getPropType(parsedType, identifier, onProp) {
        ...
        if (kind === ParsedTypeKind.Array) {
        ...
        else if (!TypeGuards.isPropValueType(type) ||
            type === PropValueType.Object ||
            type === PropValueType.Array) {
            throw new Error(`Unrecognized type ${type} in ${identifier} within ${this.studioSourceFileParser.getFilename()}`);
        }
            ...
            throw new Error(`Missing import from ${STUDIO_PACKAGE_NAME} for ${type} in ${identifier} within ${this.studioSourceFileParser.getFilename()}.`);
        }
        else if (unionValues) {
            return {
...
//# sourceMappingURL=PropShapeParser.js.map