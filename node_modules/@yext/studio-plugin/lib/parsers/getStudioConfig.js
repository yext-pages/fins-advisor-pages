 ...
 */
export default async function getStudioConfig(pathToProjectRoot) {
    try {
...
async function getStudioConfigInternal(pathToProjectRoot) {
    const defaultConfig = {
        isPagesJSRepo: false,
 ...
 * @throws {@link ParsingError|FileIOError}
 */
async function importExistingConfig(filepath) {
    let importedFile;
    try {
        importedFile = await import(/* @vite-ignore */ filepath);
    }
    catch (err) {
        const importError = new FileIOError(IOErrorKind.FailedToImportFile, `Failed to import module at ${filepath}`, err instanceof Error ? err.stack : undefined);
        throw importError;
    }
    const studioConfig = importedFile.default;
    if (!studioConfig) {
        const parsingError = new ParsingError(ParsingErrorKind.InvalidStudioConfig, "Studio Config must be a default export");
        throw parsingError;
    }
    return studioConfig;
}
