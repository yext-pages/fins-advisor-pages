...
export default function useHasChanges() {
    ...
    const siteSettingsValues = useStudioStore((store) => store.siteSettings.values);
    const siteSettingsHaveChanged = !isEqual(previousSave.siteSettings.values, siteSettingsValues);
    const hasFileMetadataChanges = !isEqual(previousSave.fileMetadatas.UUIDToFileMetadata, UUIDToFileMetadata);
    return (pagesToRemove.size > 0 ||
        pagesToUpdate.size > 0 ||
        siteSettingsHaveChanged ||
        hasFileMetadataChanges);
}
//# sourceMappingURL=useHasChanges.js.map