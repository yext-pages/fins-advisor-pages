import { PropValueType, PropValueKind, TypeGuards, TEMPLATE_STRING_EXPRESSION_REGEX, } from "@yext/studio-plugin";
 ...
 */
export function getPropsForPreview(props, propShape, expressionSources) {
    const transformedProps = {};
    Object.keys(propShape).forEach((propName) => {
        const value = getPropValueForPreview(props[propName], propShape[propName], expressionSources);
        if (value !== undefined) {
...
function getPropValueForPreview(propVal, propType, expressionSources) {
    if (!propVal) {
...
function handleNoPropVal(valueType) {
...
function handleExpressionProp(value, propType, expressionSources) {
    if (propType.type === PropValueType.Record) {
        return value;
    }
    const literalValue = getExpressionPropLiteralValue(value, propType, expressionSources);
    if (propType.type !== PropValueType.Array || Array.isArray(literalValue)) {
        return literalValue;
    }
}
function handleObjectProp(value, propType, expressionSources) {
    if (propType.type !== PropValueType.Object) {
        throw new Error(`Expected PropMetadata of type Object, received ${propType.type}`);
    }
 ...
 */
function getExpressionPropLiteralValue(value, propType, expressionSources) {
    if (value === "") {
        return value;
    }
 ...
 */
function getTemplateStringValue(templateString, propType, expressionSources) {
    const hydratedString = templateString.replaceAll(TEMPLATE_STRING_EXPRESSION_REGEX, (...args) => {
    ...
    return TemplateExpressionFormatter.getTemplateStringDisplayValue(hydratedString);
}
/**
 ...
 */
function getExpressionValue(expression, propType, expressionSources) {
    function getValueFromPath(path, parentPath) {
        const sourceObject = expressionSources[parentPath];
        if (!sourceObject) {
            console.warn(`Invalid expression source type: ${parentPath}. ` +
                `Unable to extract the desired data from path: ${path}`, expressionSources);
            return null;
        }
        const pathWithoutOptionalChaining = path.replaceAll(/\?\./g, ".");
        const newPropValue = get({ [parentPath]: sourceObject }, pathWithoutOptionalChaining);
    ...
    console.warn(`Invalid expression: ${expression}.` +
        " Expressions must reference an expression source from", expressionSources);
    return null;
}
function logInvalidExpressionWarning(value, expression, propType) {
    console.warn("Invalid expression prop value:", value, `The value extracted from the expression "${expression}" ` +
        `does not match with the expected propValueType ${propType.type}`, ...(propType.type === PropValueType.Object
        ? ["with shape", propType.shape]
        : propType.type === PropValueType.Array
            ? ["with item type", propType.itemType]
            : []));
}
//# sourceMappingURL=getPropsForPreview.js.map