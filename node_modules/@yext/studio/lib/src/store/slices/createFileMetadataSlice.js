...
const createFileMetadataSlice = (set, get) => ({
    UUIDToFileMetadata: removeTopLevelFragments(initialStudioData.UUIDToFileMetadata),
    UUIDToImportedComponent: {},
    setFileMetadata: (metadataUUID, metadata) => set((store) => {
        store.UUIDToFileMetadata[metadataUUID] = metadata;
    }),
    getFileMetadata: (metadataUUID) => get().UUIDToFileMetadata[metadataUUID],
    removeFileMetadata: (metadataUUID) => set((store) => {
        ...
        if (metadata.kind === FileMetadataKind.Module) {
            ...
            console.error("removeFileMetadata is only allowed for modules, not:", metadata.kind);
        }
    }),
    ...
    setComponentTreeInModule(metadataUUID, componentTree) {
        set((store) => {
...
function assertIsModuleMetadata(fileMetadata) {
    if (fileMetadata?.kind !== FileMetadataKind.Module) {
        throw new Error(`Expected a ModuleMetadata, instead received ${JSON.stringify(fileMetadata, null, 2)}.`);
    }
}
function assertIsComponentMetadata(fileMetadata) {
    if (fileMetadata?.kind !== FileMetadataKind.Component) {
        throw new Error(`Expected a ComponentMetadata, instead received ${JSON.stringify(fileMetadata, null, 2)}.`);
    }
}
export default createFileMetadataSlice;
//# sourceMappingURL=createFileMetadataSlice.js.map