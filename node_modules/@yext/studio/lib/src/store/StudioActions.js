import { ComponentStateKind, ComponentTreeHelpers, MessageID, TypeGuards, } from "@yext/studio-plugin";
import { v4 } from "uuid";
...
export default class StudioActions {
    ...
    updateActivePage;
    importComponent;
    constructor(getPages, getFileMetadatas, getSiteSettings, getPreviousSave, getStudioConfig) {
        this.getPages = getPages;
        this.getFileMetadatas = getFileMetadatas;
        this.getSiteSettings = getSiteSettings;
        this.getPreviousSave = getPreviousSave;
        ...
        this.createComponentState = new CreateComponentStateAction(getStudioConfig).createComponentState;
        this.updateActivePage = new UpdateActivePageAction(getPages, getStudioConfig).updateActivePage;
        this.importComponent = new ImportComponentAction(getFileMetadatas).importComponent;
    }
    getComponentTree = () => {
    ...
    getComponentState = (componentTree, uuid) => {
    ...
    getComponentHasChildren = (uuid) => {
        return this.getComponentTree()?.some((c) => c.parentUUID === uuid);
    };
    getComponentMetadata = (componentState) => {
        if (componentState?.kind !== ComponentStateKind.Standard) {
            return undefined;
        }
        return this.getFileMetadatas().getComponentMetadata(componentState.metadataUUID);
    };
    getModuleMetadataBeingEdited = () => {
        const { moduleUUIDBeingEdited, getActivePageState } = this.getPages();
        const state = this.getComponentState(getActivePageState()?.componentTree, moduleUUIDBeingEdited);
        if (!state || !TypeGuards.isModuleState(state)) {
            return undefined;
        }
        return this.getFileMetadatas().getModuleMetadata(state.metadataUUID);
    };
    updateActiveComponentProps = (props) => {
        const activeComponentState = this.getActiveComponentState();
        if (!activeComponentState) {
            console.error("Error in updateActiveComponentProps: No active component found.");
            return;
        }
        if (activeComponentState.kind === ComponentStateKind.BuiltIn ||
            activeComponentState.kind === ComponentStateKind.Fragment) {
            throw new Error("Error in updateActiveComponentProps: Cannot update props for BuiltIn or Fragment components.");
        }
        const updatedComponentState = TypeGuards.isRepeaterState(activeComponentState)
            ? {
        ...
        this.replaceComponentState(activeComponentState.uuid, updatedComponentState);
    };
    updateRepeaterListExpression = (listExpression, repeaterState) => {
    ...
    updateComponentTree = (componentTree) => {
        ...
        if (moduleMetadataBeingEdited) {
            this.getFileMetadatas().setComponentTreeInModule(moduleMetadataBeingEdited.metadataUUID, componentTree);
        }
        else if (activePageName) {
            this.getPages().setComponentTreeInPage(activePageName, componentTree);
        }
    };
    replaceComponentState = (uuidToReplace, newComponentState) => {
    ...
    removeComponent = (componentUUID) => {
        ...
        const updatedComponentTree = ComponentTreeHelpers.mapComponentTree(componentTree, (componentState, mappedChildren) => {
     ...
     */
    detachModuleInstance = (moduleMetadata, moduleState) => {
        const currentComponentTree = this.getComponentTree();
        if (!currentComponentTree) {
            return;
        }
        const updatedComponentTree = currentComponentTree.flatMap((componentState) => {
            if (componentState.uuid !== moduleState.uuid) {
                return componentState;
            }
            return ComponentTreeHelpers.mapComponentTreeParentsFirst(moduleMetadata.componentTree, (child, parentValue) => {
                return {
                    ...child,
                    uuid: v4(),
...
//# sourceMappingURL=StudioActions.js.map