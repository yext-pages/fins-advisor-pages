...
export default function DetachModuleButton(props) {
    ...
    const detachModuleInstance = useStudioStore((store) => store.actions.detachModuleInstance);
    ...
    const moduleName = ComponentStateHelpers.extractRepeatedState(state).componentName;
    const tooltipText = isRepeater
        ? "Unable to detach module instance since it is in a list"
        : "Detach";
    return (_jsx("button", { onClick: handleClick, "aria-label": `Detach Module ${moduleName}`, disabled: isRepeater, children: _jsx(ActionIconWrapper, { tooltip: tooltipText, disabled: isRepeater, children: _jsx(DetachModuleIcon, {}) }) }));
}
//# sourceMappingURL=DetachModuleButton.js.map