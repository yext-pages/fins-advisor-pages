...
export default function PreviewPanel() {
    const [componentTree, moduleUUIDBeingEdited, getComponentState] = useStudioStore((store) => [
    ...
    const state = moduleUUIDBeingEdited
        ...
        : undefined;
    const list = state && TypeGuards.isRepeaterState(state)
        ? get(pageExpressionSources, state.listExpression)
        : undefined;
    const item = Array.isArray(list) ? list[0] : undefined;
    const extractedState = state && TypeGuards.isEditableComponentState(state)
        ? ComponentStateHelpers.extractRepeatedState(state)
        : undefined;
    const parentPreviewProps = usePreviewProps(extractedState, pageExpressionSources, item);
    ...
    return (_jsx(ComponentTreePreview, { componentTree: componentTree, expressionSources: expressionSources }));
}
function usePageExpressionSources() {
    const activeEntityData = useStudioStore((store) => store.pages.activeEntityData);
